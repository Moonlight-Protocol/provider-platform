...
export function handlePostCustodialRegister(
  deps: { log: Logger },
): (ctx: Context) => Promise<void> {
  const log = deps.log.scope("postCustodialRegister");

  return async (ctx) => {
    log.info("postCustodialRegister");
    try {
      ...
      log.event("custodial account registered");
