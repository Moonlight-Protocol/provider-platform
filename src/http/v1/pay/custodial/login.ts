...
export function handlePostCustodialLogin(
  deps: { log: Logger },
): (ctx: Context) => Promise<void> {
  const log = deps.log.scope("postCustodialLogin");

  return async (ctx) => {
    log.info("postCustodialLogin");
    try {
      ...
      if (account.status === PayCustodialStatus.SUSPENDED) {
        ...
        log.error(
          new Error("login on suspended account"),
      ...
      log.debug("username", username);
      log.event("custodial login successful");
