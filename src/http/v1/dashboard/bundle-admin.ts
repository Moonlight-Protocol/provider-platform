 ...
 */
export function handlePostExpireBundles(
  deps: { log: Logger },
): (ctx: Context) => Promise<void> {
  const log = deps.log.scope("postExpireBundles");

  return async (ctx) => {
    log.info("postExpireBundles");
    ...
    if (
      ...
      ctx.response.status = Status.BadRequest;
      ctx.response.body = {
        message: "All bundleIds must be non-empty strings",
      };
      return;
    }
    ...
    if (hasIdFilter && bundleIds!.length > MAX_EXPIRE_IDS) {
      ...
      ctx.response.body = {
        message:
          `bundleIds may contain at most ${MAX_EXPIRE_IDS} entries, got ${
            bundleIds!.length
          }`,
      };
      return;
    ...
    if (hasIdFilter) {
      ...
      if (skipped > 0) {
        ...
        log.event(
          "admin expire: some bundle(s) from bundleIds were not active and were skipped",
        );
      }
    }
    ...
    log.debug("totalExpired", totalExpired);
    log.debug("ageExpiredCount", ageExpiredCount);
    log.debug("idExpiredCount", idExpiredCount);
    log.event("admin expire complete");
