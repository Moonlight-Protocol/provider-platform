...
export const PROVIDER_BASE_URL = loadOptionalEnv("PROVIDER_BASE_URL") ??
  `http://localhost:${PORT}`;
...
// Mempool
export const MEMPOOL_SLOT_CAPACITY = Number(
  requireEnv("MEMPOOL_SLOT_CAPACITY"),
);
export const MEMPOOL_EXPENSIVE_OP_WEIGHT = Number(
  requireEnv("MEMPOOL_EXPENSIVE_OP_WEIGHT"),
);
export const MEMPOOL_CHEAP_OP_WEIGHT = Number(
  requireEnv("MEMPOOL_CHEAP_OP_WEIGHT"),
);
export const MEMPOOL_EXECUTOR_INTERVAL_MS = Number(
  requireEnv("MEMPOOL_EXECUTOR_INTERVAL_MS"),
);
export const MEMPOOL_VERIFIER_INTERVAL_MS = Number(
  requireEnv("MEMPOOL_VERIFIER_INTERVAL_MS"),
);
export const MEMPOOL_TTL_CHECK_INTERVAL_MS = Number(
  requireEnv("MEMPOOL_TTL_CHECK_INTERVAL_MS"),
);
// 0 = disabled; set e.g. 86400000 (24h) to auto-expire stale bundles on startup
const _rawStartupAge = Number(
  loadOptionalEnv("MEMPOOL_STARTUP_MAX_BUNDLE_AGE_MS") ?? "0",
);
if (!Number.isFinite(_rawStartupAge) || _rawStartupAge < 0) {
  throw new Error(
    `MEMPOOL_STARTUP_MAX_BUNDLE_AGE_MS must be a non-negative number, got: "${
      loadOptionalEnv("MEMPOOL_STARTUP_MAX_BUNDLE_AGE_MS")
    }"`,
  );
}
...
if (
  !Number.isFinite(_rawMaxRetry) || !Number.isInteger(_rawMaxRetry) ||
  _rawMaxRetry < 1
) {
  throw new Error(
    `MEMPOOL_MAX_RETRY_ATTEMPTS must be a positive integer, got: "${
      requireEnv("MEMPOOL_MAX_RETRY_ATTEMPTS")
    }"`,
  );
}
...
if (
  !Number.isFinite(_rawMaxOps) || !Number.isInteger(_rawMaxOps) ||
  _rawMaxOps < 1
) {
  throw new Error(
    `BUNDLE_MAX_OPERATIONS must be a positive integer, got: "${
      requireEnv("BUNDLE_MAX_OPERATIONS")
    }"`,
  );
}
...
const _rawTxExpirationOffset = Number(
  loadOptionalEnv("TRANSACTION_EXPIRATION_OFFSET") ?? "1000",
);
if (
  !Number.isFinite(_rawTxExpirationOffset) ||
  !Number.isInteger(_rawTxExpirationOffset) || _rawTxExpirationOffset < 1
) {
  throw new Error(
    `TRANSACTION_EXPIRATION_OFFSET must be a positive integer, got: "${
      loadOptionalEnv("TRANSACTION_EXPIRATION_OFFSET")
    }"`,
  );
}
