...
import { convertRawBundleToBuffer,RawBundle } from "../../../models/bundle/bundle.schema.ts";
...
import { OPEX_HANDLER, TX_INVOCATION } from "../../../config/env.ts";
import { OPEX } from "../../opex/index.ts";
import { Buffer } from "buffer";
import { sha256Hash } from "@fifo/spp-sdk";

export const PROCESS_NEW_BUNDLE = ProcessEngine.create(async ( input: ContextWithParsedPayload <PostBundlePayload>) => {
    ...
    const loadedBundle = await db.bundles.findByPrimaryIndex("hash", bundle.hash);
    ...
    const selected = await OPEX.selectFreeUtxos(1);
    const delegateUtxoSeq = selected[0];
    const delegateUtxo = OPEX.getUtxo(delegateUtxoSeq);
    ...
    const cr = await db.bundles.add({
        ...
        delegateUtxo: Buffer.from(delegateUtxo.keypair.publicKey).toString("base64"),
        txHash: txHash,
    });
    ...
    console.log("Bundle obj: ", (await db.bundles.findByPrimaryIndex("hash", bundle.hash))?.value);
...
const formatBundle = async (
  rawBundle: RawBundle,
):  Promise<Bundle> => {
    // const rawUtxos = [...rawBundle.create, ...rawBundle.spend];
    
    const bundlBufferPreHash = POOL.buildBundlePayload(convertRawBundleToBuffer(rawBundle));
