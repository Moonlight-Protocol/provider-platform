...
export const P_UpdateChallengeSession = (deps: { log: Logger }) =>
  ProcessEngine.create(
    (
      ...
      return withSpan("P_UpdateChallengeSession", async (span) => {
        const log = deps.log.scope("P_UpdateChallengeSession");
        const sessionManager = getSessionManager(deps);
        ...
        log.debug("key", key);
        log.event("updating memory session");
